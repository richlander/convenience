...
public static class JsonWithSourceGeneration
{
    ...
    public static async IAsyncEnumerable<Release> GetReleases()
    {
        ...
        var releases = await httpClient.GetFromJsonAsync<ReleaseIndex>(ReleaseValues.RELEASE_INDEX_URL, ReleaseContext.Default.ReleaseIndex) ?? throw new Exception(loadError);
        ...
        foreach (var releaseSummary in releases.ReleasesIndex)
        {
            ...
            var release = await httpClient.GetFromJsonAsync<Release>(releaseSummary.ReleasesJson, ReleaseContext.Default.Release);
